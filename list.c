...
#define BUFSIZE 8192
#define LIMIT "50"
#define TIPS "\33[1mPlease enter the index number of folder/files(start from 0. enter -1 to return root)\33[0m\33[1;36m->\33[0m "
...
int main(int argc, char **argv) {
	...
	while (scanf("%d", &user_input) == 1 && user_input <= atoi(LIMIT) - 1) {
		if (user_input > i)
			fprintf(stderr, "%sEnter value memory overflow: %d\n", ERRMSG, i);
		if (user_input < 0) {
...
void show_file_list(const cJSON *json) {
	...
	printf("Three are \33[1;33m%u folders\33[0m, and \33[1;34m%u files\33[0m.\n", folder_count, file_count);
}